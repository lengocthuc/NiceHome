...
var cart = {
	choose_product_to_cart: function(maSanPham, soluong) {
		...
		var num = parseInt($("#numm").text())+1;
		$("#numm").html(num);
		$(".num_cart").html(num);
		$.get("/choose-product-to-cart", data, function(data, status){
			alert("Data: " + data + "\nStatus: " + status);
		});
	...
	linh_change_quantity_product: function(maSanPham, soluong,id_price,price_one) {
			...
			$("#numm").html(num);
			$(".num_cart").html(num);
			$.get("/change-product-cart", data, function(data, status){
		...
		remove_product : function(productId) {
			...
			$.get("/remove-product", data, function(data, status){
				var child = "#" + productId;
				$(child).remove();
				alert("Data: " + data + "\nStatus: " + status);
			});
...
shop = {
	...
	check_discount : function() {
		var data = {};
		data["discountName"] = $("#discount").val();

		$.get("/check-discount", data, function(data, status){
			if (data == "notfound") {
				$("#mess").css("color","red");
				...
				var total = $("#total").val();
				var disc = total - total * parseFloat(data);
